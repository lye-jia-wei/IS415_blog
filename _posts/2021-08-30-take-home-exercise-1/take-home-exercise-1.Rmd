...
Covid_Cases_Boxmap 
```
- From the boxmap, we can efficiently identify outliers which in this case highlights
that sub district in Central and Southern Jakarta region are of the upper outlier class 

- On the whole, it can be said that COVID-19 cases per 10k population in Jakarta belongs
to the '50% - 75%' class 
...
- Each sub-district has 9 neighbours, there are no municipalities with no neighbours.
...
utilised to detect spatial clusters of municipalities with high or low COVID-19 
...
```{r echo=TRUE, eval=TRUE}
head(JKT_localmoran_Mar20, 10)
```
Here is the coefficients for Local Moran’s I for Mar 2021 for the first 10 sub-district

```{r echo=TRUE, eval=TRUE}

head(JKT_localmoran_Mar21, 10)
```
...
localmoran_map <- function(df, Date) {
  ...
  tm_fill(col = "Ii", 
          ...
          title = 'Local Moran \nStatistics') +
  tm_borders(alpha = 0.5) +
  tm_layout(panel.labels = sprintf('Local Moran\'s I of %s', Date),
            panel.label.color = 'gray12',,legend.title.size = 0.75, legend.text.size = 0.75)+
  tm_compass(type="4star", size = 1.1) + 
  tm_scale_bar(width = 0.15)+
  tm_grid(lwd = 0.1, alpha = 0.2) 
}
...
```{r echo=TRUE, eval=TRUE}
moranplot_13 <- localmoran_map(JKT_localmoran_Mar20, "Mach 2020")
moranplot_32 <- localmoran_map(JKT_localmoran_Mar21, "March 2021")
...
pvalue_map <- function(df, Date) {
  ...
  tm_grid(lwd = 0.1, alpha = 0.2)+
  tm_borders(alpha = 0.5) +
  tm_layout(panel.labels = sprintf('Local Moran\'s I p-values of %s', Date),
            panel.label.color = 'gray12', legend.outside = FALSE,legend.title.size = 0.75, legend.text.size = 0.55)
}
```
...
- From the result, we can onserve that there is significant and positive local autocorrelation for COVID-19 case per 10k population at these bordering sub-district, suggesting a grouping of sub-district with COVID-19 rates that are higher than average for these bordering municipalities during March 2020.
...
There is therefore significant and positive local autocorrelation for COVID-19 cases per 10k population rate at sub-district in March 2021, suggesting that this group of sub-district are a cluster for COVID-19 at e-week 32, with COVID-19 rates that are higher than average.
